...
int main(int argc, char **argv) {
	...
	/*Phase I*/
	/*Start server*/
	attach_name = name_attach(NULL, "controller", 0);
	...
	/*Send connection message to a server des_display*/
	coid = name_open("display", 0);
	if (coid == -1) {/*If returns error*/
		perror("Error in the name_open function");
		exit(EXIT_FAILURE);
	}
	...
	name_close(coid);/*Client is done with server*/
